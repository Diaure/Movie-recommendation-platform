...
@st.cache_data
def load_data():
    df = pd.read_csv("./data/datasets/propre/df_clean4_nlp.csv", sep=',', index_col=0)
    if df is None or df.empty:
        st.error("Erreur : Le fichier CSV est vide ou introuvable.")
    return df

df_offres2 = load_data()
df_offres3 = load_data()
...
def accueil(df_offres2):
    ...
    # st.write(results)ajouter les colonnes description, profil et lien à la table filtrée utilisée pour les résultats de recherche
    ...
    if modal.is_open() and st.session_state.offre_selectionnee is not None:
        ...
        with modal.container():
            ...
            st.write(offre["details_offre"])

            
            if pd.notna(offre["Profile_origine"]):
                st.markdown("### 🎯 Profil recherché")
                st.write(offre["Profile_origine"])
            else:
                 ""
...
def analyser_tendances(df_offres3):
    import pandas as pd
    ...
    df_offres3["PublishDate"] = pd.to_datetime(df_offres3["PublishDate"])
    ...
    df_offres3["Competences_Clés"] = convertir_listes(df_offres3["Competences_Clés"])
    df_offres3["Outils"] = convertir_listes(df_offres3["Outils"])
    df_offres3["Soft_Skills"] = convertir_listes(df_offres3["Soft_Skills"])
    ...
    with col1:
        Métier = col1.selectbox("Métier", ["Tous"] + sorted(df_offres3["categorie_metier"].unique()))
        Domaine = col1.selectbox("Domaine", ["Tous"] + sorted(df_offres3["Domaine"].unique()))
    ...
    df_trend_daily = filtered_df.groupby(df_offres3["PublishDate"].dt.strftime('%Y-%m-%d')).size().reset_index(name="Nombre d'offres")
